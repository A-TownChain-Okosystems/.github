...
def impact_for(path):
    ...
    risk = (
        "P1"
        if (
            domain in ("policies", "merge_gates", "authorization", "branch_policy")
            or path.endswith("governance-rules.yaml")
        )
        else "P2"
    )
    return {
        ...
        "tests": sorted(tests),
        "workflows": sorted(wfs),
        "repositories": sorted(repos),
        "risk": risk,
        "owner_gate": "required" if risk == "P1" else "recommended",
    }


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Aufruf: gov_impact.py <geaenderte Datei>")
        sys.exit(2)
