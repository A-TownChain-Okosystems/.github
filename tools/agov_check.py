...
def check_repo(repo):
    ...
    if agents:
        ...
        findings["020"] = (
            ...
            else verdict("WARN", "Struktur dünn")
        )
    else:
    ...
    if license_text is None:
        ...
        findings["005"] = (
            verdict("PASS")
            if "Apache License" in license_text[:2000]
            else verdict("WARN", "nicht Apache-2.0")
        )

    findings["006"] = (
        verdict("PASS")
        if file_content(repo, "SECURITY.md")
        else verdict("WARN", "SECURITY.md fehlt")
    )
    findings["007"] = (
        verdict("PASS")
        if file_content(repo, "CHANGELOG.md")
        else verdict("WARN", "CHANGELOG fehlt")
    )
    findings["008"] = (
        verdict("PASS")
        if any(path.endswith("CODEOWNERS") for path in paths)
        else verdict("N/A", "MAY: nicht vorhanden")
    )
    ...
    if not workflows:
        ...
        findings["019"] = (
            verdict("PASS")
            if not write_all
            else verdict("FAIL", f"write-all: {','.join(write_all[:3])}")
        )

    suspicious_paths = [
        path
        for path in paths
        if re.search(r"\.env$|\.pem$|\.key$|id_rsa|secret", path, re.IGNORECASE)
    ]
    secret_hits = []
    ...
    has_code = any(
        re.search(
            r"Cargo\.toml$|package\.json$|pyproject\.toml$|\.py$|\.rs$|\.ts$|\.js$",
            path,
        )
        for path in paths
    )
    ...
    findings["015"] = (
        ...
        else verdict("N/A", "kein Code-Build (reines Docs-Repo)")
    )
