...
def main():
    tests = subprocess.run(
        [sys.executable, os.path.join(ROOT, "tools/test_agov.py")],
        capture_output=True,
        text=True,
    )
    t_out = tests.stdout
    ...
    ex = yaml.safe_load(open(os.path.join(ROOT, "ai/exceptions.yaml"), encoding="utf-8"))[
        "exceptions"
    ]
    ...
    print(
        f"Governance Score: {score}/100  (Tests {t_pass}/{t_total}, aktive Exceptions={active_exc})"
    )
    print(f"Debt (zulaessig, kein Blocker): {p2} P2-Drift-Findings (Org-Rollout, Roadmap)")
    print(
        "STATUS: BLOCKED — offene P1/Test-Failures existieren (Score verscholeiert nichts)"
        if blocked
