...
def decide(agent_id, op, repo=None, path=None):
    ...
    m = re.search(rf"^\|?\s*{re.escape(agent_id)}\b.*$", manifest, re.MULTILINE)
    _identity = step(
        "identity",
        bool(m),
        f"AGENT_MANIFEST-Zeile: {'gefunden' if m else 'NICHT gefunden'}",
    )
    ...
    if agent_ops is None:
        ok = step(
            "permission",
            False,
            f"kein permission-Eintrag fuer {agent_id} (Default DENY)",
        )
    else:
        op_entry = next((o for o in agent_ops["operations"] if o["op"] == op), None)
        ok = step(
            "permission",
            op_entry is not None and op_entry.get("allowed") is True,
        ...
        if ok:
            ...
            step(
                "scope",
                True,
                f"scope={op_entry.get('scope', 'beliebig')}" + (f"; note={note}" if note else ""),
            )
    # 4. Policy: ATC-POL-007 (destruktiv) / ATC-POL-009 (workflow) Owner-Gate
    _pol = load("ai/policies.yaml")["agent_policies"]
    if op in ("force_push", "delete_branch", "push_workflow_file"):
        gates = perms.get("gates", {})
        if op == "push_workflow_file" and "GH013" in str(
            gates.get("push_workflow_file", "")
        ) + note_for(agent_ops, op):
            step(
                "policy-owner-gate",
                False,
                f"{op}: Owner-Gate/GH013 — Agent darf nicht selbst (CAPABILITY-001 REQ-AGOV-CAP-003)",
            )
        else:
            step(
                "policy-owner-gate",
                False,
                f"{op}: destruktiv/workflow — Owner-Gate erforderlich (ATC-POL-007/009)",
            )
    ...
    if "--json" in args:
        print(
            json.dumps(
                {
                    "decision": d,
                    "chain": [{"step": s, "ok": ok, "evidence": e} for s, ok, e in chain],
                },
                indent=1,
            )
        )
