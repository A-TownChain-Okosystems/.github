...
check(
    "T1 checks.yaml: 20 Checks, v1.0.0, ACTIVE",
...
check(
    "T5 policies.yaml: 10 maschinenprüfbare POLs, alle enabled",
    len(p["policies"]) == 10 and all(x["enabled"] for x in p["policies"]),
)
check(
    "T6 POL-P0 existieren (no-secrets, destructive, truthful)",
...
check(
    "T7 agent.yaml: Schema 1.0, 44 Standards, 8 Rollen",
    a["schema_version"] == "1.0"
    and len(a["required_standards"]) == 44
    and len(a["roles"]["allowed"]) == 8,
)
check(
    "T8 Aurora ohne MODIFY_WORKFLOWS (GH013)",
    "MODIFY_WORKFLOWS" not in a["capabilities"],
)
...
check(
    "T9 MANIFEST-001: 10 Sektionen + Kaskade vollstaendig u. geordnet",
    m.count("## ") >= 10
    and all(p >= 0 for p in pos)
    and pos == sorted(pos)
    and "ATC-AI-GOV-MANIFEST-001" in m,
)
...
g = yaml.safe_load(open(os.path.join(ROOT, "ai/governance-rules.yaml"), encoding="utf-8"))[
    "governance_rules"
]
check(
    "T11 governance-rules: 10 Merge-Gates, NO-MERGE-Regel",
    len(g["merge_gate"]["gates"]) == 10 and "NO MERGE" in g["merge_gate"]["rule"],
)
check(
    "T12 governance-rules: Exception 6 Schritte, FAIL/BLOCKED ohne Approval",
    len(g["exception_rule"]["steps"]) == 6
    and g["exception_rule"]["without_approval"]["merge"] == "BLOCKED",
)
check(
    "T13 governance-rules: Konflikt-Hierarchie ATC-STD-000 zuerst",
    g["conflict_priority"]["order"][0] == "ATC-STD-000"
    and len(g["conflict_priority"]["order"]) == 5,
)
sn = json.load(open(os.path.join(ROOT, "ai/audit/SNAPSHOT-2026-09-09.json"), encoding="utf-8"))
check(
    "T14 Snapshot-Record: 4 Pflichtfelder, Commit+Hash korrekt",
    all(
        k in sn
        for k in (
            "registry_version",
    ...
    and len(sn["registry_commit"]) == 40
    and len(sn["registry_hash"]) == 64,
)
...
check(
    "T15 Manifest-Version konsistent (Header=Selbstreferenz=Footer)",
    bool(hdr and foot and refs)
    and foot.group(1) == hdr.group(1)
...
cnt = sc.get("governance_kpi", {}).get(
    "GOVERNED_REPOSITORY_COUNT", sc.get("organization_meta", {}).get("repository_count")
)
...
check(
    "T16 Cross-File: ai/*.yaml parsbar, Versionsschluessel, Org-Scope-SSOT konsistent (Manifest referenziert SSOT)",
...
check(
    "T17 Schema-Selbstvalidierung (6 YAML-Dateien + Snapshot gegen ai/schemas/)",
    _ok17 and _validate_schema(json.load(open(_sn_path, encoding="utf-8")), _sn_sch),
)

# T18: Authorization-Matrix gueltig (Audit-Block 5)

az = yaml.safe_load(open(os.path.join(ROOT, "ai/authorization.yaml"), encoding="utf-8"))[
    "authorization"
]
_req18 = {"agent", "instance", "repository", "operation", "paths", "approval_required"}
check(
    "T18 Authorization-Matrix: Eintrags-Schema + Agent-IDs bekannt + GH013 abgebildet",
...
check(
    "T19 Exceptions: expiry Pflicht, keine abgelaufene AKTIVE, OWNER-Genehmigung",
    all(
        e.get("expires") and e["expires"] >= _today and e.get("approved_by") == "OWNER"
        for e in ex
...
check(
    "T20 Branch-Policy: .github enforce_admins=true (F-045), keine Force-Pushes/Deletions, Defaults + Severity-Mapping",
...
regs = sorted(_g.glob(os.path.join(ROOT, "knowledge/regressions/REG-*.yaml")))
_req21 = {
    "id",
    "root_cause",
    "symptom",
...
_ok21 = (
    all(_req21 <= set(yaml.safe_load(open(p, encoding="utf-8"))) for p in regs) and len(regs) >= 4
)
check(
    "T21 Regression-KB: >=4 Eintraege mit Pflichtfeldern (root_cause/detection/prevention/regression_test)",
    _ok21,
)
...
check(
    "T22 Policy-Graph: Kanten zeigen auf existierende Knoten (Policy->Check->Workflow)",
...
check(
    "T23 Provenance: 11-stufige Kette, ID-Klassen, Commit-Trailer-Format definiert",
    pv["chain"] == _req_chain
    and all(k in pv["id_classes"] for k in ["REQUIREMENT", "STANDARD", "TASK", "CHANGE", "AUDIT"])
    and "format" in pv["commit_trailer_rule"],
)
