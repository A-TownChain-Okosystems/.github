...
def get(p):
    return requests.get("https://api.github.com" + p, headers=H)
...
yaml.dump(
    scope,
    open("ai/org-scope.yaml", "w", encoding="utf-8"),
    sort_keys=False,
    allow_unicode=True,
    default_flow_style=False,
)
print(
    f"Org: {len(repos)} · governed: {len(governed)} · ungoverned: {len(ungoverned)} → {ungoverned}"
)
