...
def workflow_perm_class(repo):
    ...
    for wf in wfs:
        ...
        has_block = re.search(r"^permissions:", body, re.MULTILINE) or re.search(
            r"^\s+permissions:", body, re.MULTILINE
        )
        write_all = (
            re.search(r"permissions:\s*\n?\s*(all|write-all)|:\s*write-all", body)
            and "write-all" in body
        )
...
def scan_repo(repo, workflows_only_ci=False):
    ...
    if agents_md:
        f["Org-Bindung (AGOV-CHECK-002)"] = (
            "PASS"
            if any(k in agents_md for k in ("AGENT_MANIFEST", ".github/AGENTS.md", "ATC-AI-GOV"))
            else "WARN"
        )
    ...
    if agent_yaml:
        f["capabilities.yaml (lokal, Pflicht bei Agenten-Bindung)"] = (
            "PASS" if file_exists(repo, ".github/ai/capabilities.yaml") else "FAIL"
        )
        f["policies.yaml (lokal, Pflicht bei Agenten-Bindung)"] = (
            "PASS" if file_exists(repo, ".github/ai/policies.yaml") else "FAIL"
        )
    else:
    ...
    f["Audit-Records"] = (
        "PASS"
        if (
            file_exists(repo, "docs/AUD-2026-0001_self_audit.md")
            or file_exists(repo, ".github/ai/audit")
            or any(True for _ in [1])
            and file_exists(repo, "docs")
        )
        else "WARN"
    )
    ...
    ci = ci_state(repo)
    f["CI letzter Lauf"] = {"success": "PASS", "failure": "FAIL"}.get(
        ci, "WARN" if ci == "running" else "N/A"
    )
    return f
...
def repo_status(findings):
    _severities = {
        "FAIL": [
            "Workflow-Permissions (AGOV-CHECK-009)",
            "Audit-Records",
        ],  # WARN-Klasse je Definition
    }
    ...
    if fails:
        return (
            ...
            fails,
            warns,
        )  # P1-FAIL (AGENTS/agent-bindings/capabilities/policies)
    if warns:
        return "COMPLIANT-WITH-FINDINGS", [], warns
    return "COMPLIANT", [], []


repos = api(f"https://api.github.com/orgs/{ORG}/repos?per_page=100&sort=pushed")
report = {
    "scan_id": f"ORG-GOV-SCAN-{datetime.now(timezone.utc).strftime('%Y-%m-%d')}",
...
json.dump(
    report,
    open(f"docs/ORG-GOV-SCAN-{datetime.now(timezone.utc).strftime('%Y-%m-%d')}.json", "w"),
    indent=1,
)
print("\nSummary:", count)
