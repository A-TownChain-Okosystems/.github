...
for rp in sorted(repos()):
    ...
    if dangling:
        gaps["P1"].append(
            (
                rp,
                f"{len(dangling)} Bindung(en) auf nicht existierende Standard-IDs: {', '.join(dangling[:5])}",
            )
        )
    if to_draft:
        gaps["P1"].append(
            (
                rp,
                f"{len(to_draft)} Bindung(en) auf DRAFT-Standard (nicht §9-freigegeben): {', '.join(to_draft[:5])}",
            )
        )
    if to_cand:
        gaps["P2"].append(
            (
                rp,
                f"{len(to_cand)} Bindung(en) auf CANDIDATE-Standards: {', '.join(to_cand[:5])}",
            )
        )
    if req and n_ok == 0:
...
if unbound:
    gaps["INFO"].append(
        (
            "ORG",
            f"{len(unbound)} APPROVED-Standards nirgends gebunden (z. B. {', '.join(sorted(unbound)[:5])})",
        )
    )
...
md = [
    f"# ATC Registry-Binding-Lücken-Report {today} (SCR-0059)",
    "",
