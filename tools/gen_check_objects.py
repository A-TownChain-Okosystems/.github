...
TYPES = {
    ...
    "002": (
        ...
        {
            "path": "AGENTS.md",
            "pattern": "AGENT_MANIFEST|\\.github/AGENTS\\.md|ATC-AI-GOV",
        },
    ),
    "003": (
        "content_match",
        {"path": "AGENTS.md", "pattern": "[Kk]askade|Org-AGENTS|Vererbung|\\.github"},
    ),
    ...
    "011": (
        "file_pattern",
        {"pattern": "requirements.txt|poetry.lock|package-lock.json|Cargo.lock|uv.lock"},
    ),
    ...
    "014": (
        ...
        {
            "path": "README.md",
            "pattern": "REALITY_STATUS|Status",
            "note": "MANUAL: Doku-Konsistenz pruefen",
        },
    ),
    ...
    "017": (
        "file_pattern",
        {"pattern": "\\.orig$|~$|\\.rej$|debug_.*\\.(log|txt)$", "expect": "0 Treffer"},
    ),
    "018": (
        "no_destructive",
        {"expect": "keine destruktiven Operationen ohne Autorisierung"},
    ),
    "019": ("workflow_permissions", {"expect": "least-privilege: kein write-all"}),
    "020": (
        "schema_valid",
        {"schema": "agents-md-structure", "note": "Struktur/Schema der AGENTS.md"},
    ),
}
...
for c in cat["agov_checks"]["checks"]:
    ...
    obj = {
        ...
        "evaluation": {
            "pass_when": [{"type": typ, **{k: v for k, v in req.items() if k not in ("note",)}}],
            "fail_closed": True,
        },
        "result": {"allowed": ["PASS", "FAIL", "WARN", "N/A"]},
        "remediation": {
            "required_on": ["FAIL"] if c["level"] == "MUST" else ["FAIL", "WARN"],
    ...
    with open(out, "w") as f:
        f.write(
            "# ATC-CHECK-%s — generiert von tools/gen_check_objects.py (Phase 2, SCR-0063)\n" % n
        )
